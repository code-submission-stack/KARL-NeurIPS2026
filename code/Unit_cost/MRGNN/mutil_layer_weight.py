...
import numpy as np
from kan_layers import MultiHeadKANAttention

class KANformerMultiplexFusion(nn.Module):
    """
    Fully KAN-ified Cross-Layer Semantic Fusion.
    Operates identically to previous aggregation schemas in I/O signature to ensure seamless integration.
    """
    def __init__(self, features_num, dropout, alpha, metapath_number, device, num_heads=4):
        super(KANformerMultiplexFusion, self).__init__()
        self.features_num = features_num
        self.metapath_number = metapath_number
        self.device = device
        
        # Layer Normalization stabilizes deep KAN grid updates during backpropagation
        self.layer_norm = nn.LayerNorm(features_num)
        
        # Initialize KANformer Attention
        self.kan_attention = MultiHeadKANAttention(
            embed_dim=features_num, 
            num_heads=num_heads, 
            grid_size=5, 
            spline_order=3
        )
        
        # Learned Initial transformation
    ...
    def forward(self, node_features, nodes_ori, layer_predict):
        """
        node_features: List of tensors from each multiplex layer.
        nodes_ori: Node indices.
        layer_predict: The target layer index currently being evaluated.
        """
        nodes_ori = np.array(nodes_ori).tolist()
        nodes = torch.tensor(np.unique(nodes_ori))
        
        # Prepare and align input features across all layers
        # Tanh activation enforces the [-1, 1] grid support optimal for B-splines
        node_features_temp = torch.zeros((self.metapath_number, node_features[0].size(0), node_features[0].size(1))).to(self.device)
        for kl in range(self.metapath_number):
            node_features_temp[kl] = torch.tanh(torch.matmul(node_features[kl], self.trans) + self.bias)
        
        # Reshape to Transformer sequence paradigm: [Num_Nodes, Num_Layers, Features]
        layer_all_attention = torch.transpose(node_features_temp, 0, 1).to(self.device)
        
        # Extract target layer features (The Query Anchor)
        target_layer_features = layer_all_attention[:, layer_predict, :].unsqueeze(1) 
        
        # Extract all other layers (The Semantic Context)
        exclude_dims = [i for i in range(self.metapath_number) if i != layer_predict]
        context_layer_features = layer_all_attention[:, exclude_dims, :] 
        
        # Apply Layer Norm
        target_layer_features = self.layer_norm(target_layer_features)
        context_layer_features = self.layer_norm(context_layer_features)
        
        # Execute B-Spline Cross-Layer Attention
        fused_context, attention_weights = self.kan_attention(
            query=target_layer_features,
            key=context_layer_features,
            value=context_layer_features
        )
        
        # Residual Integration: Target Embeddings + KAN-fused Multiplex Context
        fused_context = fused_context.squeeze(1) 
        X = node_features_temp[layer_predict] + fused_context
...
#         # FIXED: Replaced .cuda(self.device) with .to(self.device)
#         Z = torch.zeros(node_features.shape[1], node_features.shape[2]).to(self.device)
...
#     def layer_node_attention(self, node_features, i):
#         # FIXED: Replaced .cuda(self.device) with .to(self.device)
#         a_temp = torch.zeros(node_features.shape[1], 1).to(self.device)
#         a_temp[i, 0] = 1
#         layer_attention = torch.transpose(node_features, 2, 1)
#         b = layer_attention.matmul(a_temp).reshape(layer_attention.shape[0], layer_attention.shape[1])
#         return b
...
#         # FIXED: Replaced .cuda(self.device) with .to(self.device)
#         layer_semantic = torch.zeros_like(node_layer_feature).to(self.device)
#         exclude_dims = [i for i in range(self.metapath_number) if i != layer_predict]
